...
export function DiagnosticsPanel() {
  ...
  return (
    <aside className="w-64 border-l border-gray-200 bg-gray-50/50 shrink-0 overflow-y-auto">
      <div className="p-4 space-y-4">
        {/* ── Diagnostics ────────────────────────────────────── */}
        <div>
          <button
            onClick={() => setDiagOpen(!diagOpen)}
            className="flex items-center gap-2 w-full text-left text-xs font-semibold text-gray-500 uppercase tracking-wide hover:text-gray-700 transition-colors"
          >
          ...
          {diagOpen && lastCall && (
            <div className="mt-3 space-y-2.5 text-sm">
              <div>
                ...
                <p className="text-gray-700 font-medium">{lastCall.service}:{lastCall.operation}</p>
              </div>
              <div className="flex items-center gap-2">
                <p className="text-[10px] text-gray-400 uppercase tracking-wide">Status</p>
                <StatusDot status={lastCall.status} />
                <span className="text-gray-700">{typeof lastCall.status === 'number' ? lastCall.status : lastCall.status === 'ok' ? '200' : 'Error'}</span>
              </div>
              <div>
                <p className="text-[10px] text-gray-400 uppercase tracking-wide">Latency</p>
                <p className="text-gray-700">{lastCall.latencyMs} ms</p>
              </div>
              <div>
                <p className="text-[10px] text-gray-400 uppercase tracking-wide">Est. Cost</p>
                <p className="text-gray-700">{formatCost(lastCall.estimatedCost)}</p>
              </div>
              <div>
                <p className="text-[10px] text-gray-400 uppercase tracking-wide">Total Calls</p>
                <p className="text-gray-700">{totalCalls}</p>
              </div>
              {lastCall.requestId && (
                <div>
                  <p className="text-[10px] text-gray-400 uppercase tracking-wide">Request ID</p>
                  <p className="text-gray-500 text-xs font-mono break-all leading-relaxed">{lastCall.requestId}</p>
                </div>
              )}
            </div>
          )}
                  ...
                  <div className="space-y-1">
                    {Object.entries(callsByService).map(([svc, { count, cost }]) => (
                      <div key={svc} className="flex items-center justify-between text-xs">
                        <span className="text-gray-600 font-medium">{svc}</span>
                        <span className="text-gray-400">{count} calls — {formatCost(cost)}</span>
                      </div>
                    ))}
                  </div>
                </div>
