  ...
  return (
    <div>
        ...
        {TABS.map(({ id, label }) => (
          <button
            ...
            className={`px-4 py-2 text-sm font-medium rounded-t-lg transition-colors -mb-px ${
              tab === id
                ? 'bg-white dark:bg-white/[0.05] border border-gray-200 dark:border-white/10 border-b-white dark:border-b-transparent text-blue-700 dark:text-blue-400'
        ...
        <div className="border-2 border-dashed border-gray-300 dark:border-white/15 rounded-lg p-8 text-center">
          ...
          <label htmlFor="vision-upload" className="cursor-pointer">
            ...
            <p className="mt-3 text-gray-600 dark:text-gray-300">
              <span className="text-blue-600 dark:text-blue-400 font-medium">Click to upload</span> an image
            </p>
      ...
      {tab === 'analyze' && analyze.data && (
        <Card className="mt-6">
          <h2 className="text-lg font-semibold dark:text-white mb-4">Analysis Result</h2>
          ...
          {analyze.data.data.captionResult && (
            <div className="mb-6">
              ...
              <p className="text-gray-900 dark:text-white font-medium">{analyze.data.data.captionResult.text}</p>
            ...
            <div className="mb-6">
              ...
              <div className="flex flex-wrap gap-2">
                {analyze.data.data.tagsResult.values.map((tag) => (
                  <span
                    key={tag.name}
                    className="px-2.5 py-1 bg-blue-50 dark:bg-blue-900/20 text-blue-800 dark:text-blue-300 rounded-full text-sm border border-blue-100 dark:border-blue-800"
                  >
                    {tag.name}
          ...
          {analyze.data.data.objectsResult?.values && analyze.data.data.objectsResult.values.length > 0 && (
            <div>
              <p className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2">Objects</p>
              <div className="flex flex-wrap gap-2">
                {analyze.data.data.objectsResult.values.flatMap((obj) =>
                  obj.tags.map((t, i) => (
                    <span key={i} className="px-2.5 py-1 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-800 dark:text-indigo-300 rounded-full text-sm border border-indigo-100 dark:border-indigo-800">
          ...
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-6 border-t dark:border-white/10 pt-3">
      ...
      {tab === 'ocr' && ocr.data && (() => {
        ...
        return (
          <Card className="mt-6">
            <h2 className="text-lg font-semibold dark:text-white mb-4">Extracted Text</h2>
            {content ? (
              <pre className="whitespace-pre-wrap text-sm text-gray-800 dark:text-gray-200 bg-gray-50 dark:bg-white/5 rounded-lg p-4 font-mono leading-relaxed">
                {content}
              </pre>
            ) : lines.length > 0 ? (
              <div className="space-y-1">
                {lines.map((line, i) => (
                  <p key={i} className="text-sm text-gray-800 dark:text-gray-200">{line.text}</p>
                ))}
              </div>
            ) : (
              <p className="text-gray-500 dark:text-gray-400 text-sm">No text detected in this image.</p>
            )}
