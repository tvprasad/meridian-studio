...
export function Dashboard() {
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-8">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="bg-white rounded-xl border border-gray-200 border-l-4 border-l-gray-200 p-6 animate-pulse">
        ...
        <div className="mt-8 bg-white rounded-xl border border-gray-200 p-6 animate-pulse">
    ...
    return (
      <Card className="border-l-4 border-l-red-400">
        <div className="flex items-center gap-3">
          ...
          <p className="text-red-700 font-medium">Failed to connect to Meridian API</p>
        </div>
        <p className="text-red-500 text-sm mt-1 ml-8">Check that the backend is running at the configured URL.</p>
      </Card>
    );
  }
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-900">Dashboard</h1>
      <p className="text-gray-500 mt-1">A live snapshot of your Meridian deployment — pipeline status, knowledge base size, and active providers.</p>
      {health && (
      ...
      <Card className="mt-8">
        ...
        <dl className="mt-4 grid grid-cols-2 gap-4 text-sm">
          <div>
            <dt className="text-gray-500">Retrieval Provider</dt>
            <dd className="font-medium mt-0.5">{health?.retrieval_provider}</dd>
          </div>
          <div>
            <dt className="text-gray-500">LLM Provider</dt>
            <dd className="font-medium mt-0.5">{health?.llm_provider}</dd>
          </div>
        </dl>
      </Card>
    </div>
