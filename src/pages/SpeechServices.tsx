    ...
    <div>
        ...
        {TABS.map(({ id, label }) => (
          <button
            ...
            className={`px-4 py-2 text-sm font-medium rounded-t-lg transition-colors -mb-px ${
              tab === id
                ? 'bg-white border border-gray-200 border-b-white text-teal-700 dark:bg-white/[0.05] dark:border-white/10 dark:border-b-transparent dark:text-teal-400'
        ...
        <>
            ...
            <div className="border-2 border-dashed border-gray-300 dark:border-white/15 rounded-lg p-8 text-center">
              ...
              <label htmlFor="audio-upload" className="cursor-pointer">
                ...
                <p className="mt-3 text-gray-600 dark:text-gray-300">
                  <span className="text-teal-600 dark:text-teal-400 font-medium">Click to upload</span> an audio file
                </p>
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">WAV, MP3, OGG, FLAC, M4A up to 25MB</p>
              </label>
            </div>
              ...
              <div className="mt-4 flex items-center justify-between p-4 bg-gray-50 dark:bg-white/5 rounded-lg">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400">{(audioFile.size / 1024).toFixed(1)} KB</p>
                  </div>
                </div>
          ...
          {transcribe.isError && (
            <Card className="mt-4 border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-900/20">
              <p className="text-red-800 dark:text-red-300 text-sm">{(transcribe.error as Error).message}</p>
            </Card>
          )}
          ...
          {transcribe.data && (
            <Card className="mt-6">
              ...
              <p className="text-gray-800 dark:text-gray-200 leading-relaxed">{(transcribe.data as unknown as Record<string, unknown>).text as string ?? JSON.stringify(transcribe.data)}</p>
      ...
      {tab === 'tts' && (
        <>
          <Card className="mt-0 rounded-tl-none">
            <div className="flex gap-4 items-start">
              <div className="flex-1">
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-2">Text to Speak</label>
                <textarea
                  rows={5}
                  className="block w-full rounded-lg border border-gray-300 dark:border-white/15 p-3 text-sm focus:border-teal-500 focus:outline-none dark:bg-gray-900 dark:text-gray-200"
              ...
              <div className="w-48 shrink-0">
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-2">Voice</label>
                <select
                  value={voice}
                  onChange={(e) => setVoice(e.target.value)}
                  className="block w-full rounded-lg border border-gray-300 dark:border-white/15 p-2.5 text-sm focus:border-teal-500 focus:outline-none dark:bg-gray-900 dark:text-gray-200"
                >
          ...
          {tts.isError && (
            <Card className="mt-4 border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-900/20">
              <p className="text-red-800 dark:text-red-300 text-sm">{(tts.error as Error).message}</p>
            </Card>
          )}

          {audioUrl && (
            <Card className="mt-6">
              <h2 className="text-lg font-semibold mb-4 dark:text-white">Synthesized Audio</h2>
